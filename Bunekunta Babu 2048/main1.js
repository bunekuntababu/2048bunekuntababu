...
function isGameOver() {
  ...
  if (emptySquare) return { gameOver: false };

  for (let i = 0; i < board.length; i++) {
    for (let j = 0; j < board[i].length; j++) {
      if (board[i + 1] && board[i + 1][j] === board[i][j]) return { gameOver: false };
      if (board[i][j + 1] && board[i][j + 1] === board[i][j]) return { gameOver: false };
    }
  }
...
function handleSwipe() {
  ...
  if (Math.abs(deltaX) > Math.abs(deltaY)) {
    ...
    if (deltaX > 0) {
      // Swipe right
      movePossible = moveTiles(0, 1); // Right
    } else {
      // Swipe left
      movePossible = moveTiles(0, -1); // Left
    }
  } else {
    // Vertical swipe
    if (deltaY > 0) {
      // Swipe down
      movePossible = moveTiles(-1, 0); // Down
    } else {
      // Swipe up
      movePossible = moveTiles(1, 0); // Up
    }
  }

  // If the move was valid, add a new tile and check for game over
  if (movePossible) {
    addRandomTile();
    let gameOver = isGameOver();
    if (gameOver.gameOver) showAlert(gameOver.message);
